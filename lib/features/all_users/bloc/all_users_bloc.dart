...
class AllUsersBloc extends HydratedBloc<AppEvent, AppState> {
  final AllUsersRepo repo;
  final InternetConnection internetConnection;

  AllUsersBloc({required this.repo, required this.internetConnection})
      : super(Start()) {
    on<Get>(onGet);
  ...
  Future<void> onGet(Get event, Emitter<AppState> emit) async {
    if (await internetConnection.updateConnectivityStatus()) {
      ...
      print("${_engine.currentPage == 0} kkk");
      if (_engine.currentPage == 0) {
        ...
        Either<ServerFailure, Response> response =
            await repo.getAllUsers(_engine);
