...
class InterestBloc extends HydratedBloc<AppEvent, AppState> {
  ...
  InterestBloc({required this.repo, required this.internetConnection})
      : super(Start()) {
    on<Get>(onGet);
    on<Add>(onAdd);
    on<Delete>(onRemove);
  ...
  onAdd(Add event, Emitter<AppState> emit) async {
    if (await internetConnection.updateConnectivityStatus()) {
      try {
        ...
        Either<ServerFailure, Response> response =
            await repo.addtoInterest(event.arguments as int);
  ...
  onRemove(Delete event, Emitter<AppState> emit) async {
    if (await internetConnection.updateConnectivityStatus()) {
      try {
        emit(Loading());

        Either<ServerFailure, Response> response =
            await repo.removeInterest(event.arguments as int);
